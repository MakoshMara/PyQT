...
class RegisterUser(QDialog):
    ...
    def save_data(self):
        ...
        """
        print(f'Метод проверки правильности ввода и сохранения в базу нового пользователя запущен')
        try:
            if not self.client_name.text():
                ...
                # нижнем регистре.
                print(f'Пройдена проверка на ошибки')
                print(f'Генерируем хэш для {self.client_name.text}')
                ...
                print(f'Отправляем в базу {self.client_name.text}')
...
if __name__ == '__main__':
    app = QApplication([])
    database = DataBase('../server_database.db3')
